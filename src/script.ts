...
import { DataItem, Hacktiv8orData } from "./types";
...
async function fetchHacktiv8orConfig(data: DataItem[]): Promise<Hacktiv8orData> {
    let headers = { Authorization: `token ${githubToken}`, Accept: "application/vnd.github.raw+json" }
    const hacktiv8orFile = data.filter(file => file.name == ".hacktiv8or")[0]
    const url = hacktiv8orFile.git_url;
    let hacktiv8orData: Hacktiv8orData;
    if (etagCache[url]) headers["If-None-Match"] = etagCache[url];
    const response = await fetch(url, { method: "GET", headers: headers })
    if (response.status == 200) {
        etagCache[url] = response.headers.get("ETag");
        hacktiv8orData = JSON.parse(await response.text());
        etagCache[`${url}_data`] = hacktiv8orData;
    } else if (response.status == 304) hacktiv8orData = etagCache[`${url}_data`];
    return hacktiv8orData;
}
...
async function displayFolderContents(data: DataItem[], home: boolean) {
    ...
    currentContent = data;
    data = data.sort((a, b) => {
        if (a.name === ".hacktiv8or") return -1;
        if (b.name === ".hacktiv8or") return 1;
    ...
    document.getElementById("new_folder_name").addEventListener("keydown", event => {
        if (event.key == "Enter")
            uploadFileToRepo(
                { name: `${(event.target as HTMLInputElement).value}/.hacktiv8or` },
                btoa("{}")
            )
    })
    ...
    for (const item of data) {
        ...
        if (hacktiv8orData.hidden?.indexOf(item.name) > -1 || item.name == ".hacktiv8or") {
        ...
        if (adminMode && item.name !== ".hacktiv8or") {
            element.addEventListener("contextmenu", event => {
                ...
                if (hacktiv8orData.hidden?.indexOf(item.name) > -1) {
    ...
    document.getElementById("context-hide").addEventListener("click", async () => {
        ...
        fetch(hacktiv8orFile.url, {
            ...
            body: JSON.stringify({
                ...
                content: btoa(JSON.stringify(hacktiv8orData)),
                sha: hacktiv8orFile.sha
            })
